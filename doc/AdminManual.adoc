...
To set up the database connection for Open Hospital, edit and configure the database settings in the **oh.sh / oh.ps1** startup scripts or directly modify the _database.properties_ and _log4j.properties_ files, located in the _oh/rsc/_ “resource” folder.
...
Open Hospital is a Java software program and it is portable by definition, since it does not require installation. The *PORTABLE* mode refers to the possibility of deploying and executing a single instance of the OH software in a self-contained package that includes Java, MariaDB/MySQL Server and all the data. Once deployed on a single computer, the package can be copied and run everywhere - even from an USB stick - carrying together all the saved data.

"Portable" versions (where PORTABLE mode set by default) are available; see the <<download,Download>> section for more details.
...
_192.168.0.100_ – an example of a private IP address (LAN) +
_217.147.110.117_ – an example of a public IP address (WAN)
* *DBPORT*: the TCP port of the OH database Server (default _3306_)
* *DBNAME*: the database name (default _oh_)
* *DBUSER*: the database user (default _isf_)
* *DBPASS*: the database user password
