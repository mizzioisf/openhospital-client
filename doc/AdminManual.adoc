...
* *DICOM_STORAGE - dicom.manager.impl*: the Java class that implements org.isf.dicom.manager.DicomManagerInterface and that manages the DICOM persistence operation. This property accepts on of the following:
...
* *OH_PATH_SUBSTITUTE/DICOM_DIR - dicom.storage.filesystem*: if the value of the first parameter is org.isf.dicom.FileSystemDicomManager,
this property specifies the path where the DICOM files are stored
* *DICOM_SIZE - dicom.max.size*: is the maximum allowed size for a dicom/jpg image (the MariaDB/MySQL server setting max_allowed_packet should be set with the same or a larger value).
