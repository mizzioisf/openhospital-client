...
Open Hospital official releases can be downloaded at the following link:
...
on Linux:
----
/usr/local/OpenHospital-1.11.1
----
on Windows:
----
C:\Users\OH\OpenHospital-v1.11.1_
----
...
* on Linux:
----
tar zxvf OpenHospital-v1.11.1-linux_[ARCH]-[portable].tgz # portable version
tar zxvf OpenHospital-v1.11.1-linux_[ARCH]-[portable].tgz or _OpenHospital-v1.11.1-multiarch-client.zip_
* on Windows: _unzip  OpenHospital-v1.11.1-windows_[ARCH]-[portable].zip_ or _OpenHospital-v1.11.1-multiarch-client_
----

Multiarch client version

* on Linux: _unzip x OpenHospital-v1.11.1-multiarch-client.zip_
* on Windows: _unzip OpenHospital-v1.11.1-multiarch-client.zip_

After uncompressing the package, browse to the extracted directory:

----
cd _/usr/local/OpenHospital-v1.11.1 (Linux)
cd C:\Users\OH\OpenHospital-v1.11.1 (Windows)
----



NOTE: Don not run OH by double-clicking on the oh.bat startup script without unzipping the package


== 4 Installation

=== 4.1 Open Hospital - CLIENT mode
...
* Uncompress the downloaded OH package in a folder of choice
* Install the JRE - Java Runtime Environment (see compatibility matrix for the right version) on every client
* Install the MariaDB/MySQL server on a computer that will act as the OH server
* Create the OH database through the execution of a script placed in the sql/ folder
* Copy the Open Hospital application on every client (it may also be installed on the server)
* Configure each Open Hospital instance to point to the OH Database server (using the hostname or the IP address)

For the Java JRE installation refer to the next chapter <<jre,Java Runtime Environment>>
and the online documentation.

For the MariaDB/MySQL database server installation and configuration refer to the chapter <<database-server,Database Server>> and the online documentation.

For the OH database creation refer to the chapter <<ohdatabase-creation,Open Hospital - Database creation>> and the online documentation.

For the Open Hospital installation on the clients, it is sufficient to copy and extract the zip package in the desired folder.

For the Open Hospital configuration refer to the chapter <<configuration,Configuration>>.


==== 4.1.1 Prerequisites - Software compatibility matrix
...
===== 4.1.3.1 MariaDB/MySQL - Server installation
...
===== 4.1.3.2 MariaDB/MySQL - Configuration
...
===== 4.1.3.3 MariaDB/MySQL - Networking
...
[#ohdatabase-creation]
==== 4.1.4 Open Hospital - Database creation

To create the Open Hospital database, a MySQL client and a set of SQL script files placed under the sql/ folder are needed.
...
==== 4.1.5 Open Hospital - Configuration
...
NOTE: It is possibile to use the provided scripts to generate config files: on Linux execute *oh.sh -g*, on Windows launch *oh.bat* and choose the option *g*.


==== 4.1.6 Open Hospital - Startup and run
...
=== 4.2 Open Hospital - PORTABLE mode
...
[#backup-data]
==== 4.3.1 Backup of database (alternative with MySQL Workbench®)
...
[#restore-data]
==== 4.3.2 Restore of database (alternative with MySQL Workbench®)
...
==== 4.3.3 Backup of code and settings (Software)
...
[#folders]
=== 4.4 Folders and directory structure
...
This means that clients running Open Hospital may run differently according to their configuration files.
...
==== 5.1.32 ALLOWMULTIPLEOPENEDBILL
...
== 7 Installing Open Hospital 1.11.1 in Eclipse EE
...
sql (mysql for older version) folder (e.g.):
