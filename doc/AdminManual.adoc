...
Config file can be _my.ini_ or _my.cnf_;
Default for Windows MariaDB is _my.ini_ located at "C:\Program Files\MariaDB-[VERSION]\data\"
...
* the MariaDB/MySQL Server *Port Number*, if changed from the default “3306”
...
It is possibile to apply different resolutions in order to find the best setting or get information about a problem.
...
this property specifies the path where the DICOM files are stored. It is possible to specify a local folder (data/dicom_storage is the default path under the OH installation folder) or to set a shared network folder, userful for LAN - client / server environment. Suggested configuration is to map the network/NFS folder under the data/dicom_storage path with a symbolic link.
The shared folder needs read/write permissions for uploading DICOM images.
