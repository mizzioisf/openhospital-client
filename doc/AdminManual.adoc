= Open Hospital Administrator’s Guide 1.11.1
:icons: font
:stem:
:toc: left
:toclevels: 4
...
These specifications are for client only; database server specifications may vary accordingly to the different use cases.
...
Backup of the client *data and configuration files*, especially for "PORTABLE" mode, can be performed by saving/copying the *"data"* folder (including all subfolders) placed under the main OH installation folder.
...
NOTE: The properties files are automatically generated/overwritten at any **oh.sh / oh.ps1**  startup, unless MANUAL_CONFIG option is set to "on" in the scripts.
...
Default configuration is set in the _settings.properties.dist_ file:
...
In the following sections the use and meaning of each KEY/value pair is documented.
...
NOTE: The OH_LANGUAGE setting is automatically generated/overwritten at any **oh.sh / oh.ps1**  startup, unless MANUAL_CONFIG option is set to "on" in the scripts.
...
NOTE: The DOC_DIR setting is automatically generated/overwritten at any **oh.sh / oh.ps1**  startup, unless MANUAL_CONFIG option is set to "on" in the scripts.
...
Default configuration is set in the _database.properties.dist_ file:
...
Use the provided startup scripts in order to automatically generate the properties file from the corresponding .dist files, or set manually the following parameters:
...
* *DBPORT*: the TCP port of the OH database Server
...
NOTE: The _database.properties_ file is automatically generated/overwritten at any **oh.sh / oh.ps1**  startup, unless MANUAL_CONFIG option is set to "on" in the scripts.
...
NOTE: The properties files are automatically generated/overwritten from the corresponding .dist files at any **oh.sh / oh.ps1**  startup, unless MANUAL_CONFIG option is set to "on" in the scripts.
...
Default configuration is set in the _log4j.properties.dist_ file:
...
NOTE: The same settings must be applied in _database.properties_, see
<<database-properties,database.properties>> in this manual.

Once the system is up and running, the log level can be reduced from "DEBUG" to "INFO", in order to avoid large log files:
