= Open Hospital 1.12.1 - Administrator’s Guide
<<<
:icons: font
:stem:
:toc: left
...
32 bit (i686) and 64 bit (x86_64) architectures are supported: 64 bit architecture is recommended.
...
NOTE: Do not double-click on the **oh.bat** startup script without unzipping the package first!

A compressed archive can be opened on most operating systems by just double-clicking on it.
In this way, the archive is just opened, but not uncompressed.
...
After uncompressing the package, browse to the extracted directory (example given for version 1.12.1):

* on Linux:
----
cd /home/OH/OpenHospital-v1.12.1
----
* on Windows:
----
cd C:\Users\OH\OpenHospital-v1.12.1
----
...
More information can be found on the Open Hospital web site https://www.open-hospital.org/documentation.
...
* <<client,2.1 OH - CLIENT mode>>: multi computer setup, with a central database (requires a little IT knowledge)
* <<portable,2.2 OH - PORTABLE mode>>: single computer setup, with the database on the same computer (click and run)
* <<server,2.3 OH - SERVER mode (portable)>>: multi computer setup, with the central database created by the Open Hospital script (click and run)

TIP: PORTABLE mode can be also used to test the application.

[#client]
=== 2.1 OH - CLIENT mode

In this mode, the application will need a Server with a DBMSfootnote:[DataBase Management System] to store the data. 

The free and open-source MariaDBfootnote:[See https://www.mariadb.org] is the preferred database server.
...
* Server 
** <<download,Download>> the Open Hospital package for the selected Operating System (see <<prereq,Prerequisites>>)
** <<unpack,Unpack>> the downloaded package on the server machine by uncompressing the archive in a folder of your choice
** <<database-server,Install and configure MariaDB/MySQL>> on the server/computer that will act as the database server
** <<ohdatabase-creation,Create the database>> through the execution of a script found in the _sql/_ folder
** <<setup,Test>> and <<run,run>> the application using 'localhost' setting
* Clients
** Unpack the downloaded package on a client machine in a folder of your choice
** <<setup,Configure>> and <<run,run>> the application to point to the database server using the hostname or the IP address
** Copy the newly configured package on every client
...
NOTE: See OH Software Compatibility Matrix to identify the correct Java version.
...
NOTE: See OH Software Compatibility Matrix to identify the correct MariaDB / MySQL version.
...
Please refer to the latest MariaDB/MySQL Server online documentation for downloadfootnote:[https://mariadb.org/download/] and installationfootnote:[https://mariadb.org/documentation/] information while using this document.
...
[source,shell]
----
C:\WINDOWS> cd C:\Users\OH\OpenHospital-v1.12.1\sql

C:\Users\OH\OpenHospital-v1.12.1\sql
...
[source,shell]
----
C:\Users\OH\OpenHospital-v1.12.1\sql> mysql -u root -p [-h hostname/ip address]
Enter password: *****
----
...
we are now ready to create the database user (isf) with password, the main OH database (oh) and grant the necessary permissions with the following SQL commands:
...
NOTE: "isf123" is used here as the default password. Change it for production setup!
...
+--------------------+
----
The next step is to use the created database and launch the main SQL database creation script with the command:
...
*Alternative method*: you can first generate the config files with the *-g* and after directly modify the _database.properties_ and _log4j.properties_ configuration files, located in the _oh/rsc/_ resource folder, without generating them every time.

Please set them according to the values specified in the installation phase:

in _database.properties_ config file
```
...
in _log4j.properties_ config file
```
...
More information on these files can be found in the <<configuration,3. Configuration>> section.
...
image:OH-icon.png[OH-icon,width=32,height=32]
...
include::https://raw.githubusercontent.com/informatici/openhospital/master/oh-bundle/OH-README.md[lines=28..429,leveloffset=+2]

[#server]
=== 2.3 OH - SERVER mode (portable)

Open Hospital can be launched in SERVER (portable) mode. SERVER mode launches a local instance of the database server and listens for connections from other clients.
This mode is useful to test/simulate a client/server configuration or networked environment; it is not suggested for production setup.
To setup a complete client / server configuration, refer to chapter <<database-server,2.1.3 - Database Server>>
...
NOTE: The easiest way is to use the provided scripts: on Linux execute *oh.sh -e*, on Windows launch *oh.bat* and choose the option *e*.
...
[#backup-data]
==== 2.4.1 Backup of database (alternative with MySQL Workbench®)
...
image:image22a.png[image,width=642,height=461]

[#restore-data]
==== 2.4.2 Restore of database (alternative with MySQL Workbench®)
...
==== 2.4.3 Backup of OH code and settings
...
** _zulu11.<version>-<operating_system>_<arch>/ - Java Virtual Machine
** _mariadb-<version>-<arch>/ or _mysql-<version>-<arch>-<build>/_ - MySQL Server or MariaDB Server

=== 2.6 Appearance

Open Hospital main menu appearance can be customized by adding a logo of choice. To test the feature, rename the image file _logo_hospital_example.png_ found in the _oh/rsc/images_ folder to _logo_hospital.png_:

image:OH-main_menu_logo.png[OH main menu]
...
NOTE: The properties files can be automatically generated/overwritten at any Open Hospital startup, by setting the WRITE_CONFIG_FILES option to "on" in the **oh.sh / oh.ps1** scripts.
...
Use the provided startup scripts in order to automatically generate the _settings.properties_ file from the corresponding _.dist_ file; after generation, optionally adjust the KEY/value pair as explained in the following sections.
...
|LANGUAGE |en |ar, de, en, es, fr, it, pt, sw, zh_CN
|===
...
NOTE: The language (OH_LANGUAGE) can be automatically set by using the provided **oh.sh / oh.ps1** scripts.
...
image:image27.png[UsersListLogin.png,width=375,height=139]

NOTE: See <<users-list-login, 3.1.36 USERSLISTLOGIN>> in this document to change the login mode.
...
This means that it is possible to work in the pharmacy without taking care of lot definitions in loading the store and
...
NOTE: The DOC_DIR setting can be automatically generated/overwritten at any OpenHospital startup, by setting the WRITE_CONFIG_FILES option to "on" in the **oh.sh / oh.ps1** scripts.
...
Open Hospital includes a feature that allows attaching image files to a patient’s clinical sheet.
When this option is enabled, an *Imaging* button is shown in the patient Clinical sheet module.
...
From this window it is possible to have more information about webcams connected to the system.
...
|===
|Key |Default Value | Allowed Values
|STRONGPASSWORD |yes | yes, no
|===

By default, the value of STRONGPASSWORD is the set to yes indicating that the
password must contain at least one alphabetic, numeric, and special character.
The list of recognized special characters is: *_$&+,:;=\?@#|/'<>.^*()%!-*
...
|===
|Key |Default Value | Allowed Values
|STRONGLENGTH |10 | positive integer value
|===
...
|===
|Key |Default Value | Allowed Values
|PASSWORDTRIES |5 | a positive integer value greater than or equal to 5
|===

The value of PASSWORDTRIES is the maximum number of consecutive failed passwords when attempting to log in before the account is locked.
...
|===
|Key |Default Value | Allowed Values
|PASSWORDLOCKTIME | 60 | a positive integer value greater than or equal to 60; the unit of time is minutes
|===

The value of PASSWORDLOCKTIME is the number of minutes that an account is locked after failing to specify the correct password the number of times specified by
the PASSWORDTRIES variable described above.
...
NOTE: The maximum size for patient's profile picture is limited to 32,768 bytes. If a bigger image is provided, it will automatically resized.
...
|===
|Key |Default Value | Allowed Values
|SESSIONTIMEOUT |5 | positive integer value
|===
...
|===
|Key |Default Value | Allowed Values
|EXAMINATIONCHART |patient_examination | any kind of .jasper file name
|===

Open Hospital can produce a report about patient_examinations.
(see link:https://github.com/informatici/openhospital-doc/blob/develop/doc_user/UserManual.adoc#8-5-2-patient-examination[8.5.2 Patient Examination] in the User's Guide).

By default, EXAMINATIONCHART is set to patient_examination which is the filename of the related report to use for the *Patient Examination* functionality.
It is possible to use a different report by installing it in the report folder (see <<reports,Reports>>) and by changing this parameter.
...
|===
|Key |Default Value | Allowed Values
|VISITSHEET |WardVisits | any kind of .jasper file name
|===

By default, VISITSHEET is set to WardVisits which is the filename of the related report to use for the *Visit Sheet* functionality.
It is possible to use a different report by installing it in the report folder (see <<reports,Reports>>) and by changing this parameter.
...
Use the provided startup scripts in order to automatically generate the _database.properties_ file from the corresponding _.dist_ file; after generation, optionally adjust the following parameters:
...
NOTE: The _database.properties_ file can be automatically generated/overwritten at any Open Hospital startup, by setting the WRITE_CONFIG_FILES option to "on" in the **oh.sh / oh.ps1** scripts.

=== 3.3 dicom.properties

Imaging configuration is set in the _dicom.properties_ file.
A default imaging configuration is available in the _dicom.properties.dist_ file:
...
Use the provided startup scripts in order to automatically generate the _dicom.properties_ file from the corresponding _.dist_ file; after generation, optionally adjust the following parameters:
...
this property specifies the path where the image files are stored. It is possible to specify a local folder (_data/dicom_storage_ is the default path under the OH installation folder) or to set a shared network folder, useful for LAN - client/server environment. Suggested configuration is to map the network/NFS folder under the _data/dicom_storage_ path with a symbolic link.
The shared folder needs read/write permissions for uploading images. Always use slash "/" in the path variable and not backslash "\".
...
NOTE: The _dicom.properties_ file can be automatically generated/overwritten at any **oh.sh / oh.ps1**  startup, by setting the WRITE_CONFIG_FILES option to "on" in the **oh.sh / oh.ps1** scripts.
...
Use the provided OH startup scripts in order to automatically generate the _log4j.properties_ file from the corresponding _.dist_ file; after generation, optionally adjust the following parameters:
...
NOTE: The _log4j.properties_ file can be automatically generated/overwritten at any Open Hospital startup, by setting the WRITE_CONFIG_FILES option to "on" in the **oh.sh / oh.ps1** scripts.
...
XMPP module configuration is set in the _xmpp.properties_ file:
...
Open Hospital reports are divided in three different folders, i.e. rpt_base/, rpt_stat/ and rpt_extra/ folder (see <<folders,Folders>> in this document):
...
* some reports could also have more than one *.properties* file that works for the localization, it means that the report has been translated to appear in more languages (default is English). At least the default *.properties* file should contain the *jTitle* property that defines the name visualized in the Open Hospital application, otherwise the application will not show the report (this can be useful for subreports).
...
== 5 Installing Open Hospital 1.12.1 in Eclipse EE

The OH source code is available on GitHub at the following links:

* https://github.com/informatici/openhospital-core/tree/v1.12.1[openhospital-core]
* https://github.com/informatici/openhospital-gui/tree/v1.12.1[openhospital-gui]
* https://github.com/informatici/openhospital-doc/tree/v1.12.1[openhospital-doc]
...
are already installed on the computer, this chapter will focus on cloning version 1.12.1 into the Eclipse J2EE (or Eclipse EE)
environment.
...
*Major Releases*:
Changes are documented in the RELEASE_NOTES file.

=== 6.1 Update Database

Major OH releases usually require changes to the database structure, and these changes are implemented via different SQL scripts included in the
_sql/update__ folder (e.g.):

[cols=",,",options="header",]
|*Previous version* |*New version*|* *Update script*|
|OpenHospital 1.8.4 |OpenHospital_1.9|update_1_8_4-1_9.sql|
|OpenHospital 1.9 |OpenHospital 1.10|update_1_9-1_10.sql|
|OpenHospital 1.10 |OpenHospital 1.10|update_1_10-1_11.sql|
|OpenHospital 1.11 |OpenHospital 1.12.0|update_1_12_0-1_12_1.sql|
|OpenHospital 1.12.1 |OpenHospital 1.12.1|update_1_12_0-1_12_1.sql|

To perform the update process follow these steps:
...
[source,shell]
----
C:\WINDOWS\system32> cd C:\Users\OH\OpenHospital.1.12.1\sql\update

C:\Users\OH\OpenHospital.1.12.1\sql\update>
----
...
[source,shell]
----
C:\Users\OH\OpenHospital.1.12.1\sql\update> mysql –u root -p [-h hostname/ip address]
...
[start=7]
. Execute all the SQL scripts that have been added from the previous version; for example:

[source,shell]
----
MariaDB> source update_1_8_4-1_9.sql

MariaDB> source update_1_9-1_10.sql

MariaDB> source update_1_10-1_11.sql

MariaDB> source update_1_11-1_12_0.sql

MariaDB> source update_1_12_0-1_12_1.sql
...
If the database upgrade procedure is successful, it is possibile to update the OH software by replacing the OH working copy on each client with the new one and connect it to the same DB
(see <<database-properties,database.properties>> and <<log4j-properties,log4j.properties>>).
...
[arabic]
. Close Open Hospital;
. use oh.sh / oh.ps1 **e** option to export database
...
For support, questions about updating, migrating, or maintaining an Open Hospital installation, please check our website at https://www.open-hospital.org.
You can contact us at: https://www.open-hospital.org/contact and specify:
