...
Minimum hardware requirements for running Open Hospital:
...
Open Hospital does not require installation. It is sufficient to unzip the downloaded package in a folder of choice, e.g.
----
/home/OH/OpenHospital-[VERSION]		# Linux
C:\Users\OH\OpenHospital-[VERSION]	# Windows
----
...
unzip OpenHospital-v[VERSION]-multiarch-client.zip à multiarch client version
----

After uncompressing the package, browse to the extracted directory (example given for version 1.11.1):

----
cd /home/OH/OpenHospital-v1.11.1	# Linux
cd C:\Users\OH\OpenHospital-v1.11.1	# Windows
----


NOTE: You need read/write permission on the folder of choice.
...
The information needed to use the OH software is not included in this manual; please refer to the _User’s Guide_.
...
Open Hospital is a client application that can be used on a single computer (PORTABLE mode) or in a client/server configuration (CLIENT mode). The server part utilizes a DBMSfootnote:[DataBase Management System] to archive, share, and present the information. The free and open-source MariaDBfootnote:[See https://www.mariadb.org] is the database server of choice.
...
* <<download,Download>> the Open Hospital package for the Operating System of choice
* <<setup,Setup>> the downloaded OH package by uncompressing the archive in a folder of choice
...
* <<ohdatabase-creation,Create the OH database>> through the execution of a script placed in the _sql/_ folder
...
*on a Linux machine*: during the installation, a “root” password must be defined; it is very important to choose it carefully and to keep it safe. If the password is not set during installation an empty password will be automatically set for the root user.
...
• Select Enable networking and write down the Port Number in an installation document
...
The prompt means that a successful connection to the database server instance that was just installed has been established.
...
Config file can be my.ini or my.cnf;
Default for Windows MariaDB is my.ini located at "C:\Program Files\MariaDB-[VERSION]\data\"
...
To create the Open Hospital database, a MySQL client and a set of SQL script files placed under the _sql/_ folder are needed.
...
NOTE: Remember to set the updated database password also in the Open Hospital configuration files and startup scripts.


==== 2.1.5 OH - Database Configuration

To set up the database connection for Open Hospital, edit and configure the database settings in the oh.sh/oh.ps1 startup scripts or directly modify the _database.properties_ and _log4j.properties_ files, located in the “resource” folder rsc/.
...
* the “isf” user (or the user specified in the installation phase) *password*
...
NOTE: It is possible to use the provided scripts to generate config files: on Linux execute *oh.sh -g*, on Windows launch *oh.bat* and choose the option *g*.
...
Open Hospital can also be executed in a *PORTABLE* mode. A "portable" version is also distributed, in order to have all the needed software in one single package.

Since Open Hospital is a Java software program, it is portable by definition. The “PORTABLE” mode here refers to the possibility of having a self-contained copy of the OH software that includes Java, MariaDB/MySQL Server, and the database. This package can be copied and run everywhere, carrying together all the saved data.
...
include::https://raw.githubusercontent.com/informatici/openhospital/master/oh-bundle/OH-README.md[lines=21..218,leveloffset=+2]
...
Backup of the Open Hospital *code and settings* can be performed by saving the *oh/* folder. Configuration files are placed under the *oh/rsc* folder.
...
Configuration of the local Open Hospital instance (CLIENT and PORTABLE mode) can be adapted by setting the desired parameters in the text files called “properties files”. Open Hospital uses the following properties files:
...
The files can be modified using a text editor or through the configuration options available in the startup scripts **oh.sh / oh.ps1**; the scripts can be used to generate the correct _.properties_ files from the default _.properties.dist_ filess packaged with the OH distributions.

NOTE: Any change to these files requires an application restart to apply the settings.

NOTE: The properties files are automatically generated at any **oh.sh/oh.ps1**  startup, unless MANUAL_CONFIG option is set to "on" in the scripts.
...
Open Hospital documentation is link:https://github.com/informatici/openhospital-doc[online] and PDF versions are compiled
and published at each release, and put in a folder that must be accessible by the application; this folder might change
...
To set up a GSM device, the GSM mode has to be defined in the sms.properties file (see <<sms-properties,sms.properties>>).
...
see <<log4j-properties,log4j.properties>>) and try to use the SetupGSM utility (launch the OH startup script with the "G" option: oh.sh -G / oh.bat -> select "G").
(see <<folders,Folders>>).


===== 3.1.23.2 SetupGSM utility

To use the SetupGSM utility (oh.sh -G / oh.bat -> select *G* option) just follow these instructions:
...
==== 3.1.32 ALLOWMULTIPLEOPENEDBILL
...
Open Hospital comes with a _database.properties_ file with the following defaults:
...
The remaining parameters may need modification according to the network settings.

* *jdbc.drivers*: the Java library used to connect to a database server
* *jdbc.url*: the address to point to the MySQL Server
* *jdbc.server*: the IP address of the database server (where MariaDB/MySQL Server is installed);
...
Open Hospital comes with a _dicom.properties.dist_ file set as follows:
...
In order to generate the correct _dicom.properties_ file, the following parameters need to be set:

* *dicom.manager.impl=org.isf.dicom.manager.DICOM_STORAGE*: the Java class that implements org.isf.dicom.manager.DicomManagerInterface and that manages the DICOM persistence operation. This property accepts on of the following:
** org.isf.dicom.SqlDicomManager: DICOM files are stored in the database
** org.isf.dicom.FileSystemDicomManager: DICOM files are stored in the file system
* *dicom.storage.filesystem=OH_PATH_SUBSTITUTE/DICOM_DIR*: if the value of the first parameter is org.isf.dicom.FileSystemDicomManager,
this property specifies the path where the DICOM files are stored
* *dicom.max.size=DICOM_SIZE*: is the maximum allowed size for a dicom/jpg image (the MariaDB/MySQL server setting max_allowed_packet should be set with the same or a larger value).
...
Open Hospital comes with the _examination.properties_ file used in the examination module.
...
The default _log4j.properties_ file is:
...
The default _sms.properties_ file is:
...
The default _txtPrinter.properties_ file is:
...
Open Hospital comes with the _xmpp.properties_ file set as follows by default:
...
[start=7]
. If any error message occurs you can report it to: +
...
(see <<database-properties,database.properties>> and <<log4j-properties,log4j.properties>>).
Remember to mantain the configuration files that have been changed, to preserve the custom settings.
