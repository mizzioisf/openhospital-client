...
See the <<xmpp-properties,xmpp.properties>> chapter for instructions on how to setup and configure the XMPP Server.
...
(see link:https://github.com/informatici/openhospital-doc/blob/develop/doc_user/UserManual.adoc#11-communication-communication[11 Communication] in the User’s Guide), 
an XMPP Server must be installed and configured separately on a server/computer in the local network. The XMPP server can also be installed on the same machine as the database server.
