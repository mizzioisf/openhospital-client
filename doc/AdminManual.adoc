...
* Create the OH database through the execution of a script in the folder sql/ within the .zip file folders
* Copy the Open Hospital application on every client (it may be installed on the server also)
* Configure each Open Hospital instance to point to the OH server (using the hostname or the IP address)
...
NOTE: Following instructions only applies between “official releases”.

New releases, as well as this Admin Manual and the User Manual, are regularly released on Github/SourceForge.

See the <<Download,Download>> section for more details.
