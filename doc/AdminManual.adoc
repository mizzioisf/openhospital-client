...
[source,shell]
----
C:\Users\OH\OpenHospital.1.12.0\sql> mysql –u root -p
...
=== 6.2 Update Open Hospital - software

If the databae upgrade procedure is successful, it is possibile to update the OH software by replacing the OH working copy on each client with the new one and connect it to the same DB
(see <<database-properties,database.properties>> and <<log4j-properties,log4j.properties>>).
...
. <<restore-data,Restore of data (Database)>> onto the new database server;
