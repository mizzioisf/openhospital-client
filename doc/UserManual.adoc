...
In the following chapters, all the information needed to correctly and efficiently use Open Hospital are presented. 
More information can be found on the Open Hospital web site https://www.open-hospital.org.

NOTE: The information needed to install, configure or administer the OH software is not included in this manual; please refer to the _Administrator’s Guide_ supplied with the software.
...
.The default Main Menu of the application.
[#default-main-menu-4]
[caption="Main Menu: "]
image:image0_DefaultMainMenu.png[Default Main Menu,width=250,height=627]
...
* In the case the selected patient personal data need to be updated, it is possibile to click on the Edit icon image:image15.png[Edit icon,width=26,height=26] near his/her name; a new window shows the patient personal information ready for modification.
...
===== 6.1.2.11 Print Receipt Function
...
* *Period*: set the lenght of therapy; to do it select days, weeks, and months; in the example above, the therapy is 11 days long (4 days and 1 week)
...
This function is used to chat and share information with other users logged in the system if in turn the application has been set in [.underline]#multi-user# mode and an [.underline]#XMPP Server# is active and running (Ask the Administrator or refer to the __Administrator’s Guide_).
...
<<<
[#settings]
== 11 Settings ([.underline]##S##ettings)
...
In editing mode, the type and code cannot be changed and the the only modifications allowed are to the Description and if the disease should appear only in OPD, or Admissions, or in both.
